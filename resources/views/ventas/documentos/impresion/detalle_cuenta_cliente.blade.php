                ...
                @foreach ($cuentas as $item)
                    <tr>
                        <td style="text-align: center; border-right: 2px solid #52BE80">
                            {{ getFechaFormato($item->fecha_doc, 'd/m/Y') }}</td>
                ...
                <td style="width: 60%">
                    @if ($empresa->condicion_id == 1)
                        <table class="tbl-qr">
                            <tr>
                                <td>
                                    <p class="m-0 p-0" style="color: #229954;"><em>¡¡Gracias por su confianza y
                                            preferencia!!</em></p>
                                    <div style="width: 90%; text-align: right;">
                                        <img src="{{ public_path() . '/img/gota.png' }}"
                                    ...
                                    <td>
                                        @foreach ($cuentas_bancarias as $cuenta_bancaria)
                                            <p class="m-0 p-0 text-cuerpo"><b
                                                    class="text-uppercase">{{ $cuenta_bancaria->banco_nombre }}</b>
                                                {{ $cuenta_bancaria->moneda }} <b>N°: </b>
                                                {{ $cuenta_bancaria->nro_cuenta }} <b>CCI:</b>
                                                {{ $cuenta_bancaria->cci }}</p>
                                        @endforeach
                                    </td>
                                </tr>
                            </table>
                    ...
                    <table class="tbl-total text-uppercase">
                        <tr>
                            <td style="text-align:left; padding: 5px;">
                                <p class="m-0 p-0">Saldo: S/.</p>
                            </td>
                            <td style="text-align:right; padding: 5px;">
                                <p class="p-0 m-0">{{ number_format($cuentas->sum('saldo'), 2) }}</p>
                            </td>
                        </tr>
                        <tr>
                            <td style="text-align:left; padding: 5px;">
                                <p class="p-0 m-0">Total: S/.</p>
                            </td>
                            <td style="text-align:right; padding: 5px;">
                                <p class="p-0 m-0">{{ number_format($cuentas->sum('monto'), 2) }}</p>
                            </td>
                        </tr>
                    </table>
                </td>
