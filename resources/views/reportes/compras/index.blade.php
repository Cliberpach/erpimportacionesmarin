...
@section('compras_reporte-active', 'active')
...
<script>
    ...
    function loadTable(proveedor_id,fecha_ini,fecha_fin)
    {
        ...
        $('.dataTables-reporte').DataTable({
            ...
            'bAutoWidth': false,
            "ajax": {
                "url": "{{route('reporte.compras.documento.getTable')}}",
                "type": "POST",
    ...
    function reportePdf(id) {
        var url = '{{ route("compras.documento.reporte", ":id")}}';
        url = url.replace(':id', id);
        window.open(url, "Compra SISCOM", "width=900, height=600")
    }
    ...
    function excelTable()
    {
        ...
        if(verificar)
        {
            window.location = '/reportes/compras/documento/getExcel?fecha_ini='+fecha_ini+'&fecha_fin='+fecha_fin+'&proveedor_id='+proveedor_id;
        }
        return false;
    }
</script>
