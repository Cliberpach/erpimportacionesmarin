...
@section('nota_salida_reporte-active', 'active')

<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10 col-md-10">
        <h2 style="text-transform:uppercase"><b>Listado de productos por notas de salida</b></h2>
        <ol class="breadcrumb">
...
<script>
    ...
    function initTable()
    {
        ...
        if(verificar)
        {
            loadTable(producto_id,destino,fecha_ini,fecha_fin)
        }
        return false;
    }

    function loadTable(producto_id,destino,fecha_ini,fecha_fin)
    {
        ...
        $('.dataTables-reporte').DataTable({
            ...
            'bAutoWidth': false,
            "ajax": {
                "url": "{{route('reporte.notas.salida.getTable')}}",
                "type": "POST",
                "data": {'_token' : $('input[name=_token]').val(), 'fecha_ini' : fecha_ini, 'fecha_fin' : fecha_fin, 'producto_id' : producto_id, 'destino':destino}
            },
            "columns": [
                {
                    data: 'fecha',
                ...
                },
                {
                    data: 'destino',
                    className: "text-left",
                    name:"nota_salidad.destino"
                },
                {
                    data: 'cantidad',
                    className: "text-left",
                    name:"detalle_nota_salidad.cantidad"
                }
