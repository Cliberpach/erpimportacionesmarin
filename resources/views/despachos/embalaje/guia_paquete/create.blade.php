        ...
        <div class="col-lg-12">
            <div class="ibox">
                <div class="ibox-content">
                    @include('despachos.embalaje.guia_paquete.forms.form_guia_create')
                </div>
            </div>
        </div>
    </div>
                                ...
                                <div class="panel-body ibox-content">
                                    <div class="row">
                                        ...
                                        <div class="col-12 d-flex justify-content-end">
                                            <button class="btn btn-success" type="submit" form="formRegistrarGuia">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        loadSelect2();
        $('#modalidad_traslado').val('01').trigger('change');
    ...
    function cambiarModalidadTraslado(modalidad) {
        ...
        const transportistas = @json($transportistas);
        let conductoresNew = [];
        ...
        if (modalidad === '01') {
            conductoresNew = transportistas;
            document.querySelector('#divTransportista').style.display = 'flex';
            document.querySelector('#divCategoriaML').style.display = 'none';
            conductoresNew.forEach(opcion => {
                const texto = `${opcion.tipo_documento_nombre}:${opcion.nro_documento} - ${opcion.nombre}`;
                let newOption = new Option(texto, opcion.id, false, false);
                selectConductor.append(newOption);
            });
        }
    ...
    async function getColoresTallas() {
        ...
        if (producto_id && almacen_id) {
            try {
                const res = await axios.get(route('ventas.guiasremision.getColoresTallas', {
                    almacen_id,
                    producto_id
                }));
                if (res.data.success) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    formData.append('ventas', JSON.stringify(@json($ventas)));
                    formData.append('almacen', @json($almacen_origen->id));
                    formData.append('motivo_traslado', "01");
                    formData.append('cliente', @json($cliente->id));
                    formData.append('envio_id',@json($envio->id));

                    const res = await axios.post(route('despachos.embalaje.storeGuiaEnvio'), formData);
