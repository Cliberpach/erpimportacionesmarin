<form action="" method="post" id="formRegistrarGuia">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <i class="fas fa-file-invoice"></i> <label for="" style="font-weight: bold;">DATOS DEL
                COMPROBANTE</label>
        ...
        <div class="col-lg-3 col-md-4 col-sm-12 col-xs-12">
            <div class="form-group">
                <label style="font-weight: bold;" class="required" for="condicion_id">ALMACÉN DE LAS VENTAS</label>
                <select disabled required onchange="cambiarAlmacen(this.value)" id="almacen" name="almacen"
                    class="select2_form form-control" required>
                    <option></option>
                    @foreach ($almacenes as $almacen)
                        <option @if ($almacen->id == $venta->almacen_id) selected @endif value="{{ $almacen->id }}">
                            {{ $almacen->descripcion }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12 mb-3">
            ...
            $hoy = date('Y-m-d');
            ?>
            <input required id="fecha_emision" name="fecha_emision" type="date" class="form-control"
                min="<?= $hoy ?>" value="<?= $hoy ?>">
    ...
    <div class="row mb-3">
        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
            ...
            <select disabled onchange="cambiarMotivoTraslado(this.value)" required name="motivo_traslado"
                id="motivo_traslado" class="select2_form">

                @foreach ($motivos_traslado as $motivo_traslado)
                    <option @if ($motivo_traslado->simbolo === '01') selected @endif value="{{ $motivo_traslado->simbolo }}">
                        {{ $motivo_traslado->descripcion }}</option>
                @endforeach
        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
            ...
            <select required onchange="cambiarModalidadTraslado(this.value)" name="modalidad_traslado"
                id="modalidad_traslado" class="select2_form">
                <option value=""></option>
        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
            ...
            <div class="input-group">
                ...
                <input required value="0.1" readonly name="peso" id="peso" type="text"
                    class="form-control" placeholder="Peso" aria-label="Username" aria-describedby="basic-addon1">
            </div>
        ...
        <div class="col-12 mb-3" id="divCategoriaML">
            <div class="form-group form-check">
                <input id="tipo_vehiculo" name="categoria_M1L" type="checkbox"
                    class="form-check-input chkTipoVehiculo">
    ...
    @include('despachos.embalaje.guia_paquete.subforms.sub_ubigeo')

    <div class="row mt-5">
        <div class="col-12 mb-2">
            <h5 class="font-weight-bold text-secondary">
                <i class="fas fa-file-alt"></i> Documentos Afectados
            </h5>
            <hr class="mt-1 mb-3">
        </div>
        <div class="col-12">
            <div class="table-responsive">
                @include('despachos.embalaje.guia_paquete.tables.tbl_docs_afectados')
            </div>
        </div>
    </div>
