...
<script>
    ...
    function pintarProvincias(provincias, provincia_id) {
        ...
        if (provincia_id === 'first' && data.length > 0) {
            provinciaSelect.setValue(parseInt(data[0].value));
        } else if (provincia_id) {
            provinciaSelect.setValue(parseInt(provincia_id));
        }
    ...
    async function getDistritos(provincia_id, distrito_id) {
        try {
            ...
            toastr.error(error,'ERROR EN LA PETICIÓN GET DISTRITOS');
        }
    }
    ...
    async function actualizarEnvio() {
        ...
        const validacion = validacionActualizarEnvio();
        if (!validacion) return;
        try {

            Swal.fire({
                title: 'Actualizando despacho...',
            ...
            const formDespachoData = {
                ...
                tipo_envio: document.querySelector('#tipo_envio').value,
                tipo_pago_envio: document.querySelector('#tipo_pago_envio').value,
                empresa_envio: document.querySelector('#empresa_envio').value,
                sede_envio: document.querySelector('#sede_envio').value,
                entrega_domicilio: document.querySelector('#check_entrega_domicilio').checked ? 'SI' : 'NO',
                direccion_entrega: document.querySelector('#direccion_entrega').value,
                origen_venta: document.querySelector('#origen_venta').value,
                fecha_envio: document.querySelector('#fecha_envio').value,
                obs_rotulo: document.querySelector('#obs-rotulo').value,
                obs_despacho: document.querySelector('#obs-despacho').value,
                tipo_doc_destinatario: document.querySelector('#tipo_doc_destinatario').value,
                nro_doc_destinatario: document.querySelector('#nro_doc_destinatario').value,
                nombres_destinatario: document.querySelector('#nombres_destinatario').value,
                despacho_id: parametrosMdlEnvioEdit.id
            };
            const res = await axios.post(route('ventas.despachos.actualizarDespacho'), formDespachoData);
            ...
            toastr.error(error, 'ERROR EN LA PETICIÓN ACTUALIZAR DESPACHO');
        } finally {
            Swal.close();
        }
    ...
    function validacionActualizarEnvio() {
        let validacion = true;
