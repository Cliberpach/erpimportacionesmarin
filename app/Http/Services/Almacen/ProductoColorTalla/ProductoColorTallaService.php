...
class ProductoColorTallaService
{
    ...
    {
        $items  =   Detalle::where('documento_id', $venta_id)->where('tipo', 'PRODUCTO')->get();
    ...
    public function analizarStock(array $items): array
    {
        $list   =   [];
        foreach ($items as $item) {
      ...
      public function decrementarStockFisico(int $almacen_id, int $producto_id, int $color_id, int $talla_id, float $cantidad)
    {
        DB::update(
            'UPDATE producto_color_tallas
                        SET
